...
class Bola extends Arc {
    private _velocidadeX: number;
    private _velocidadeY: number;
    constructor(
    ...
    private colisaoParede(jogador?: Raquete, oponente?: Raquete, synth?: any) {
        if (this.x + this.raio > this.canvas.width) {
            ...
            if (jogador) jogador.setPontos = jogador.getPontos + 1;
            ...
            this.resetVelocidade();
            if (synth) synth.triggerAttackRelease("C4", "8n");
            if (oponente) oponente.setPontos = oponente.getPontos + 1;
        }

        if (this.y + this.raio > this.canvas.height) {
            this.velocidadeY *= -1;
            this.y = this.canvas.height - this.raio;
            if (synth) synth.triggerAttackRelease("E1", "8n");
    ...
    private isColisaoRaquete(raquete: Raquete) {
    ...
    private colisaoRaquete(raquete: Raquete) {
        this.velocidadeX *= -1;
        const deltaY =
            (this.y - (raquete.getY + raquete.getAltura / 2)) /
            (raquete.getAltura / 2);
    ...
    private resetVelocidade() {
        this.velocidadeX = this._velocidadeX;
        this.velocidadeY = Math.random() > 0.5 ? 2 : -2;
    }

    private criarExplosao(particulasExplosao: Particula[], raquete: Raquete) {
        const x = this.x;
        const y = this.y;
    ...
    update(
        ...
        hitSynth?: any,
        scoreSynth?: any,
        particulasExplosao?: Particula[]
    ) {
        this.x += this.velocidadeX;
        this.y += this.velocidadeY;
        if (this.isColisaoRaquete(jogador)) {
            if (hitSynth) hitSynth.triggerAttackRelease("E2", "8n");
            ...
            if (hitSynth) hitSynth.triggerAttackRelease("C2", "8n");
            if (particulasExplosao)
